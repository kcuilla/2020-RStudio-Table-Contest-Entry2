...
FIVETHIRTYEIGHT_playoffs <- FIVETHIRTYEIGHT_table %>%
  ...
  select("Team" = x_5,
         ...
         "Win Division" = playoff_chances_2) %>%
  mutate(`Make Playoffs` = case_when(
    `Make Playoffs` == "<1%" ~ "0.1",
...
NFL_table_clean <- NFL_table %>%
  ...
  mutate_at(c("SRS", "OSRS", "DSRS", "PF", "PA", "MoV", "SoS", "Make Playoffs"),
            as.numeric) %>%
...
interactive_plot <-
  plot_ly(
    ...
    hovertemplate = paste(
      ...
      "Playoff Odds: <b>%{marker.size}%</b><br>",
      "<extra></extra>"
    ),
    marker = list(
      size = ~ Playoffs,
...
div(
  div(class = "title",
      h2("2020 NFL Team Stats & Ratings")),
  class = "text",
      div(style = "text-align: right",
    "Note: Bubble size represents team's playoff odds according to FiveThirtyEight"),
  bscols(
    widths = c(3, NA),
    list(
      srs_filter,
      ...
      Team = colDef(
        ...
        Playoffs = colDef(
          ...
          cell = function(value) {
            ...
            value <- ifelse(value == 0.1, " <1%", paste0(value, "%"))
